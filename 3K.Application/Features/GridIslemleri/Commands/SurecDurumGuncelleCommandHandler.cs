        ...
        public async Task<Result> Handle(SurecDurumGuncelleCommand request, CancellationToken cancellationToken)
        {
            ...
            if (aktarilanKaynakSatirIdleri.Count > 0)
            {
                return Result.Failure(
                    ...
                    "Süreç işlemini saha projesindeki ürün üzerinden yapın; kaynak ürünü değiştirmek için önce saha aktarımını geri alın.",
                    409);
            }
            ...
            if (kilitliSatirIdleri.Any())
                return Result.Failure($"Seçili ürünlerden {kilitliSatirIdleri.Count} tanesi sevk edilmiş sandıkta olduğu için süreç durumu değiştirilemez.");
