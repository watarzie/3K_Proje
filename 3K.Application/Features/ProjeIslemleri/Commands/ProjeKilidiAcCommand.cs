...
{
    public class ProjeKilidiAcCommand : IRequest<Result>, ISecuredRequest, IAlwaysRequireApproval, IApprovalOperation, IApprovalReference
    {
    ...
    public class ProjeKilidiAcCommandHandler : IRequestHandler<ProjeKilidiAcCommand, Result>
    {
        ...
        public async Task<Result> Handle(ProjeKilidiAcCommand request, CancellationToken cancellationToken)
        {
            ...
            // Kilit denetimi, yalnız düzeltme bayrağının değiştiğini özgün değerlerden doğrular.
            var sandiklar = await _readQueries.ToListAsync(
                sandikRepo.Queryable().Where(s => s.ProjeId == request.ProjeId),
                cancellationToken);
            ...
            var fizikselSevkEdilmisSandikVarMi = sandiklar.Any(s =>
                s.DurumId == (int)SandikDurum.Sevkedildi);
            var yalnizSahaUzerindenSevkEdilmisNormalProje =
                proje.ProjeTipiId == (int)ProjeTipi.Normal &&
                !fizikselSevkEdilmisSandikVarMi &&
                kaynakSandikSahaDurumu.SahaUzerindenSevkEdilenSandikIds.Count > 0;

            if (yalnizSahaUzerindenSevkEdilmisNormalProje)
            {
                return Result.Failure(
                    "Bu projenin sevkiyatı yalnızca saha projesi üzerinden gerçekleşti. Ana proje kilidi açılamaz; önce ilgili saha sevkiyatını geri alın.",
                    409);
            }
            ...
            foreach (var sandik in sandiklar)
            {
                if (sandik.DurumId == (int)SandikDurum.Sevkedildi)
                {
                    ...
                    // Takipli sandıkta gerçek değişiklik EF tarafından algılanır; zaten açık
                    // bir düzeltme bayrağı için gereksiz tüm-alan Update yapılmaz.
            ...
            if (proje.DurumId != eskiDurum || proje.GerceklesenSevkTarihi != eskiGerceklesenSevkTarihi)
                repo.Update(proje);
